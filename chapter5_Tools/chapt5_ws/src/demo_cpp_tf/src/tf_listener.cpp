...
class TFListener : public rclcpp::Node {
 public:
  TFListener() : Node("tf_listener") {
    buffer_ = std::make_unique<tf2_ros::Buffer>(this->get_clock()); //对缓冲进行初始化
    listener_ = std::make_shared<tf2_ros::TransformListener>(*buffer_, this); //订阅TransformListener，将数据放到缓冲器里
